...
import '../../../home/logic/models/addproplem_request_body.dart';
import '../../../problem_review/logic/cubit/emergency_problem_response_cubit.dart';
import '../widgets/add proplem_bloc_listener.dart';
...
class ImageDetailsViewProplem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final GlobalKey<FormState> formKey = GlobalKey<FormState>(); // Define form key
    final descriptionController = context.read<AddProblemCubit>().descriptionController;
              ...
              child: Form( // Wrap in Form widget
                ...
                child: Column(
                  children: [
                    ...
                    verticalSpace(10),
                    DescrbtionTextFild(
                      contentPadding: EdgeInsets.symmetric(horizontal: 20.w, vertical: 100.h),
                      controller: descriptionController, // Connect to controller
                      hintText: "",
                      validator: (value) {
                    ...
                    AppTextButton(
                      onPressed: () {
                        if (formKey.currentState?.validate() == true) { // Validate the form
                          context.read<AddProblemCubit>().addProblem(context, XFile(image.path));
                        }
                      },
                      buttonText: S.of(context).ConfirmBtn,
                      textStyle: AppTextStyles.font16WhiteBold,
                    ),
                    const AddProblemBlocListener(),
                  ],
                ),
              ),
            ),
